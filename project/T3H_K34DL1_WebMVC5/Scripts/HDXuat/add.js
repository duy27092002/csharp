$(document).ready(function () {
        ...
        $.each(cbs, function (i, e) {
            if ($(e).is(':checked')) {
                ...
                html += '<tr>' +
                    ...
                    '<td>' + $(tds[2]).text() + '</td>' +
                    '<td> <input class = "form-control num text-center bg-white" readonly value = "' + $($(tds[4]).children(0)).val() + '"/></td>' +
                    '<td> <input class = "form-control num text-center bg-white" readonly value = "' + $($(tds[5]).children(0)).val() + '"/></td>' +
                    '<td> <input class = "form-control num text-center bg-white" readonly value = "' + $($(tds[6]).children(0)).val() + '"/></td>' +
    ...
    $('body').on('keyup', '.num', function () {
        ...
        if (parseInt(formatString($($(tds[3]).children(0)).val())) < parseInt(formatString($(this).val()))) {
            $(this).val($($(tds[3]).children(0)).val());
        }
    ...
    $('#btn-submit').click(function () {
        ...
        // tạo ra danh sách đối tượng chi tiết hóa đơn xuất
        ...
        success: function (result) {
            ...
            $.each(data, function (i, e) {
                html += '<tr>' +
                    ...
                    '<td>' + e.name + '</td>' +
                    '<td>' + e.producer + '</td>' +
                    '<td> <input class = "form-control num text-center" readonly value = "' + formatNumber(e.count) + '" /></td>' +
                    '<td> <input class = "form-control num text-center" /></td>' +
                    '<td> <input class = "form-control num text-center" readonly value = "' + formatNumber(e.unitPrice) + '" /></td>' +
...
function getTotal() {
    ...
    $('#total').text("Thành Tiền: " + formatNumber(total));
}

function add(info, details) {
    $.ajax({
        url: "/HoaDonXuat/CreateConfirmed",
        type: "Post",
        dataType: "json",
        data: {
            info: info,
        ...
        success: function (result) {
            ...
            window.location = "/HoaDonXuat/Index";
        },
        error: function (data) {
            console.log("error");
        }
